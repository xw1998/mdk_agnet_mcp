...
| 手工武装后**仍然**送不出，寄存器也确认写对了 | 楔死在**固件之下**（核心/PHY/时钟域），别再改固件逻辑，先找"什么能把它复位回来" |
...
上述楔死**跨软件复位存活**：核软复位（`CSRST`）+ 完整重初始化 + 重新枚举之后依然送不出，
只有**断电重插**才恢复。因此**排查这类现象时先要一次断电重插作为状态清零**，否则会把
"上一轮的楔死状态"误读成"这一版固件还是坏的"——本次就为此浪费了多轮真机时间。
