...
def group_ab():
    ...
    check("A4 status 自述一致（core 已装载 / 收起 156 / 注册 200 / 来源 default）",
          st.get("ok") and st.get("loaded_groups") == ["core"]
          and st.get("hidden") == 156 and st.get("total_registered") == 200
    ...
    check("B6 toolsets=all 一次全装到 200", r4.get("ok") and r4.get("exposed") == 200, r4)
...
def group_c():
    ...
    for spec, want, label in (("all", "200", "MDKDEBUG_TOOLSETS=all 仍是全开"),
                              ("serial", "20", "=serial 只留 14 串口 + 6 常驻"),
                              ("core,mem", "55", "=core,mem 组合生效"),
                              ("bogus", "200", "=bogus 组名认不出 → 不裁剪（宁可少裁不错杀）")):
...
def group_d(srv_all):
    ...
    check("D8 annotate.check_surface 在 200 个工具上无问题", not bad, bad)
