...
def group_ab():
    ...
    check("A4 status 自述一致（core 已装载 / 收起 143 / 注册 181 / 来源 default）",
          st.get("ok") and st.get("loaded_groups") == ["core"]
          and st.get("hidden") == 143 and st.get("total_registered") == 181
          and st.get("source") == "default", {k: st.get(k) for k in
                                              ("loaded_groups", "hidden", "total_registered", "source")})
    g = st.get("groups") or {}
    check("A5 status 列全 11 个组且各组规模正确",
          len(g) == 11 and g.get("core", {}).get("size") == 34
          and g.get("trace", {}).get("size") == 30 and g.get("ocd", {}).get("size") == 17
    ...
    check("A6 status 给出隐藏工具清单与装回来的办法",
          len(st.get("hidden_tools") or []) == 143 and "toolset" in (st.get("hint") or ""), "")

    cap = call_sync(srv, "capabilities", {})
    su = cap.get("tool_surface") or {}
    check("A7 capabilities 如实报注册总数 / 收起数 / 未装载组",
          su.get("registered_total") == 181 and su.get("hidden") == 143
    ...
    check("B6 toolsets=all 一次全装到 181", r4.get("ok") and r4.get("exposed") == 181, r4)
...
def group_c():
    ...
    for spec, want, label in (("all", "181", "MDKDEBUG_TOOLSETS=all 仍是全开"),
                              ("serial", "18", "=serial 只留 14 串口 + 4 常驻"),
                              ("core,mem", "49", "=core,mem 组合生效"),
                              ("bogus", "181", "=bogus 组名认不出 → 不裁剪（宁可少裁不错杀）")):
...
def group_d(srv_all):
    ...
    check("D8 annotate.check_surface 在 181 个工具上无问题", not bad, bad)
