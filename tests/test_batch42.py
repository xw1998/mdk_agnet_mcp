...
def group_ab():
    ...
    check("A4 status 自述一致（core 已装载 / 收起 147 / 注册 189 / 来源 default）",
          st.get("ok") and st.get("loaded_groups") == ["core"]
          and st.get("hidden") == 147 and st.get("total_registered") == 189
    ...
    check("A5 status 列全 11 个组且各组规模正确",
          ...
          and g.get("trace", {}).get("size") == 34 and g.get("ocd", {}).get("size") == 17
    ...
    check("A6 status 给出隐藏工具清单与装回来的办法",
          len(st.get("hidden_tools") or []) == 147 and "toolset" in (st.get("hint") or ""), "")

    cap = call_sync(srv, "capabilities", {})
    su = cap.get("tool_surface") or {}
    check("A7 capabilities 如实报注册总数 / 收起数 / 未装载组",
          su.get("registered_total") == 189 and su.get("hidden") == 147
    ...
    check("B6 toolsets=all 一次全装到 189", r4.get("ok") and r4.get("exposed") == 189, r4)
...
def group_c():
    ...
    for spec, want, label in (("all", "189", "MDKDEBUG_TOOLSETS=all 仍是全开"),
                              ("serial", "20", "=serial 只留 14 串口 + 6 常驻"),
                              ("core,mem", "53", "=core,mem 组合生效"),
                              ("bogus", "189", "=bogus 组名认不出 → 不裁剪（宁可少裁不错杀）")):
...
def group_d(srv_all):
    ...
    check("D8 annotate.check_surface 在 189 个工具上无问题", not bad, bad)
