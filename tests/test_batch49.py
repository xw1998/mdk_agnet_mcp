...
"板上真实是什么"**：
  ...
  ④ 同类复查：通用工具里不许写死型号/内存布局（本轮修掉 is_code_address 的
   ...
   A 工具面：注册总数 186 / 三新工具归组 / 注解覆盖 / 默认仍只暴露 40
...
def section_a():
    ...
    check("A8 check_surface 在 186 个工具上无问题", not bad, bad)
    # 默认面：core + ALWAYS，其余收起
    exposed = gsize("core") + len(TB.ALWAYS)
    check("A9 默认暴露 core 34 + 常驻 6 = 40（新增工具进 core 后自动生效）",
          exposed == 40, exposed)
    check("A10 默认收起 146（186-40）", 186 - exposed == 146, 186 - exposed)
