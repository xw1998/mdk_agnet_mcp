...
async def group_e_surface(server, uv4):
    ...
    check("E1 工具总数 197（工具面只增不减；批次34 +1，批次36 +46，批次40 +3，批次42 +1）", len(names) == 197, len(names))
    ...
    check("E10 capabilities：报出内置模块与工具数",
          ...
          and (r.get("tool_surface") or {}).get("tool_count") == 197, r)
    ...
    out2 = subprocess.run([sys.executable, "-c", code], cwd=ROOT, env=env2,
                          capture_output=True, text=True, encoding="utf-8",
                          errors="replace", timeout=180)
    check("E20 未知组名时不裁剪（宁可少裁不错杀；“197”即当前全量工具数）",
          out2.stdout.strip().endswith("197"),
