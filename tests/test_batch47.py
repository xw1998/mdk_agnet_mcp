...
def section_a():
    ...
    check("A1 注册工具总数 181（分组表 174 + 常驻元工具 4）", total == 181, total)
    ...
    check("A3 annotate.check_surface 在 181 个工具上无问题", not bad, bad)
    ...
    check("A6 组规模：build 16 / target 7 / trace 30",
          group_size("build") == 16 and group_size("target") == 7
          and group_size("trace") == 30,
