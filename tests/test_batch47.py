...
def section_a():
    ...
    check("A1 注册工具总数 184（分组表 174 + 常驻元工具 4）", total == 184, total)
    ...
    check("A3 annotate.check_surface 在 184 个工具上无问题", not bad, bad)
    ...
    check("A6 组规模：build 16 / target 7 / trace 33",
          group_size("build") == 16 and group_size("target") == 7
          and group_size("trace") == 33,
