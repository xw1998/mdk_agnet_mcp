...
def section_a():
    ...
    check("A1 注册工具总数 199（分组表 191 + 常驻 6）", total == 199, total)
    ...
    check("A3 annotate.check_surface 在 199 个工具上无问题", not bad, bad)
    ...
    check("A6 组规模：build 16 / target 7 / trace 37",
          group_size("build") == 16 and group_size("target") == 7
          and group_size("trace") == 37,
          {"build": group_size("build"), "target": group_size("target"),
           "trace": group_size("trace")})
    check("A7 12 个组都有用途说明", len(TB.GROUP_NOTES) == 12, sorted(TB.GROUP_NOTES))
