...
def section_a():
    ...
    check("A1 注册工具总数 174（分组表 169 + 常驻元工具 4）", total == 174, total)
    ...
    check("A3 annotate.check_surface 在 174 个工具上无问题", not bad, bad)
    ...
    check("A6 组规模：build 16 / target 7 / trace 25",
          group_size("build") == 16 and group_size("target") == 7
          and group_size("trace") == 25,
