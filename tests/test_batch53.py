...
就是看着像样、其实是别的东西的事件流：
  ...
  A 工具面：注册总数 188 / 归 trace 组 / 只读注解 / 默认暴露 42
...
def section_a():
    ...
    check("A4 check_surface 在 188 个工具上无问题", not bad, bad)
    exposed = gsize("core") + len(TB.ALWAYS)
    check("A5 默认暴露 42（批次59 两个可视化工具进 core，仍在默认面）", exposed == 42, exposed)
    check("A6 默认收起 146（188-40）", 188 - exposed == 146, 188 - exposed)
...
def section_m():
    ...
    check("M6 README 写的是 188 个工具、收起 146",
          "188 个" in readme and "146 个" in readme and "178 个" not in readme
    ...
    check("M8 SKILL.md 工具数 188、收起 146、只读清单里有 trace_eventrec",
          "188 个工具" in skill and "146 个" in skill and "trace_eventrec" in skill, "")
