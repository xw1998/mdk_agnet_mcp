...
就是看着像样、其实是别的东西的事件流：
  ...
  A 工具面：注册总数 189 / 归 trace 组 / 只读注解 / 默认暴露 42
...
def section_a():
    ...
    check("A2 trace_eventrec 归 trace 组（组规模 26->27，批次55 再加 3 个 buff 到 30，批次56 再加 3 个 swd 到 33，批次64 再加 1 个任务表到 34）",
          "trace_eventrec" in (TB.TOOLSETS.get("trace") or [])
          and gsize("trace") == 34, gsize("trace"))
    ...
    check("A4 check_surface 在 189 个工具上无问题", not bad, bad)
    exposed = gsize("core") + len(TB.ALWAYS)
    check("A5 默认暴露 42（批次59 两个可视化工具进 core，仍在默认面）", exposed == 42, exposed)
    check("A6 默认收起 147（189-40）", 189 - exposed == 147, 189 - exposed)
...
def section_m():
    ...
    check("M6 README 写的是 189 个工具、收起 147",
          "189 个" in readme and "147 个" in readme and "178 个" not in readme
    ...
    check("M8 SKILL.md 工具数 189、收起 147、只读清单里有 trace_eventrec",
          "189 个工具" in skill and "147 个" in skill and "trace_eventrec" in skill, "")
