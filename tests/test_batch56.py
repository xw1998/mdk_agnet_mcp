...
另一半是**每个工具的描述**（186 个工具 ~9.5 万字符，八成是参考手册式长文）。
本批给三条路：
  ...
  * **两个装卸元工具**：`tools_groups` / `tools_load`（参数比 `toolset` 更少）。
  ...
  A 工具面：注册总数 186 / 常驻元工具 6 / 默认暴露 40 / nano 19
...
def section_a():
    ...
    allnames = names(srv)
    check("A1 注册总数 186", len(allnames) == 186, len(allnames))
    ...
    check("A4 annotate 对全部 186 个工具无问题",
    ...
    try:
        ...
        dn = names(d)
        check("A6 默认暴露 40 个（core 34 + 常驻 6）", len(dn) == 40, len(dn))
        ...
        check("A9 status 自述一致（暴露 40 / 收起 146 / 注册 186）",
              st.get("exposed") == 40 and st.get("hidden") == 146
              and st.get("total_registered") == 186, st)
    finally:
        if saved is not None:
...
def section_i():
    ...
    check("I3 README 工具数同步为 186（186 个 + 收起 146）",
          "186 个" in readme and "146 个" in readme and "184 个" not in readme, "")
    check("I4 SKILL 写了 nano 档与描述分层",
          "nano" in skill and "MDKDEBUG_DESC" in skill and "mdk_guide" in skill, "")
    check("I5 SKILL 工具数同步为 186", "186 个工具" in skill and "184 个工具" not in skill, "")
