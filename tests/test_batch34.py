...
async def group_e_surface(server):
    ...
    check("E1 工具总数 145（批次36 再 +46）", len(names) == 145, len(names))
    ...
    check("E7 直调 max_lines：列表 5 条而 count 仍是全量（并说明计数字段口径）",
          len(t5.get("tools") or []) == 5 and t5.get("count") == 145 and
    ...
    check("E9 full=true 直调取回全量 145 条且无 output",
          big.get("count") == 145 and len(big.get("tools") or []) == 145 and "output" not in big,
          big.get("count"))
