...
async def group_e_surface(server):
    ...
    check("E1 工具总数 195（批次36 再 +46，批次40 +3，批次42 +1）", len(names) == 195, len(names))
    ...
    check("E7 直调 max_lines：列表 5 条而 count 仍是全量（并说明计数字段口径）",
          len(t5.get("tools") or []) == 5 and t5.get("count") == 195 and
    ...
    check("E9 full=true 直调取回全量 195 条且无 output",
          big.get("count") == 195 and len(big.get("tools") or []) == 195 and "output" not in big,
          big.get("count"))
