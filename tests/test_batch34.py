...
async def group_e_surface(server):
    ...
    check("E1 工具总数 99（批次34 新增 session_state）", len(names) == 99, len(names))
    ...
    check("E7 直调 max_lines：列表 5 条而 count 仍是全量（并说明计数字段口径）",
          len(t5.get("tools") or []) == 5 and t5.get("count") == 99 and
    ...
    check("E9 full=true 直调取回全量 99 条且无 output",
          big.get("count") == 99 and len(big.get("tools") or []) == 99 and "output" not in big,
          big.get("count"))
