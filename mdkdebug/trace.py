...
def register(server, js=None) -> int:
    ...
    @server.tool(
        ...
        description=(
            ...
            "留空不动粒度。\n"
            "**粒度不是越小越好，也不是越大越省事，它同时决定两件事**：① 每事件字节数"
            "（实测 500us ≈ 1.01 B、10us ≈ 1.31 B、1us ≈ 2.33 B、cycle ≈ 3.33 B），"
            "字节数上去就更容易跟不上环的容量而丢事件；② 时间轴能不能走——目标算 dt 是"
            "「与上一条的差 ÷ 粒度」并**丢掉余数**，粒度比事件平均间隔（实测百微秒级）"
            "粗时每条都算成 0，宿主累加出来的时间轴会**冻在原点**（页面看着正常、"
            "数据全错）。要真时间轴就把粒度选到比事件间隔细一档（实测 10us 是可用的"
            "甜点），要只是数事件、不要时间轴就 none。"
        ),
    )
