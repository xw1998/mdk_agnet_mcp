...
档位（`MDKDEBUG_DESC` 环境变量，或由调用方显式传入）：
  * `lean`（**默认**，批次74 起）—— 正文保留到 ~360 字符（优先在段落边界断开），
    其余归档。截断同时保住三样东西：正文的**段首 + 段末**（末尾往往是最要紧的
    告警）、结构化尾块（【输出控制】/【参数】）整段、以及一行**取回指针**。
    为什么把默认从 full 改成它：另一路 AI 反馈「工具规模太大不利于上下文与注意力
    机制」，而默认面（core 44 个）实测 8.9 万字节里 66% 是描述，其中绝大多数是
    「参考手册」式背景叙述——留指针按需取回即可，决定「用哪个工具、怎么调」的
    信息（用途、参数、风险）一个不少。批次56 曾因「截断会砍掉边界条件与失败模式」
    把默认定回 full；批次74 的判断是「入口比全文常驻更划算」，且
    `MDKDEBUG_DESC=full` 随时能退回逐字节一致的历史行为。
  * `full` —— 不做任何改写，与批次73 及以前逐字节一致（排查 / A/B 对照用）。
  * `min` —— 正文只留一句话摘要，其余归档（nano 极简档默认走这一档）。
...
MODES = ("full", "lean", "min")
# 默认档（批次74 起）：lean。对外默认暴露的描述也走分层——理由见上方档位说明。
DEFAULT_MODE = "lean"
LEAN_LIMIT = 360
...
# 取回指针（批次74 瘦身）：旧文案把「边界条件、失败模式、真机上踩过的坑」逐个列了
# 一遍，约 110 字符；每个被分层的工具都要带一句——默认面 44 个就是近 1 万字节。
# 压到 ~65 字符，保留三件必要信息：挪出了什么（概述）、怎么取回（工具名 + 参数名）、
# 以及**是哪个工具**（name）。旧的逐项罗列属于重复（"完整说明"四个字已经涵盖）。
def _pointer(name: str) -> str:
    return ('\n【完整说明】省上下文已挪出（含边界条件与失败模式）：'
            'mdk_guide(topic="tool", name="%s")' % name)
